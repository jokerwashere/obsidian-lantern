...
export interface AgentTool {
	...
	/**
	 * True for tools that mutate the vault or need interactive confirmation
	 * (the write tools). The agent loop runs read-only tools concurrently but
	 * keeps a batch containing any mutating tool sequential, so Apply/Deny cards
	 * don't stack and writes stay ordered.
	 */
	mutates?: boolean;
}
