	...
	private renderMain(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addExtraButton((btn) =>
				btn
					...
					.onClick(async () => {
						const models = await this.plugin.listLlmModels();
						if (models.length === 0) {
		...
		new Setting(containerEl)
			...
			.addSlider((slider) =>
				slider
					.setLimits(0, 0.7, 0.05)
					.setValue(this.plugin.settings.agentMinScore)
					.setDynamicTooltip()
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addExtraButton((btn) =>
				btn
					...
					.onClick(() => this.createPromptNote((path) => promptNoteText.setValue(path)))
			);
