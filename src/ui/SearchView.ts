...
import { LANTERN_ICON, createAnswerNowIcon } from "./lanternIcon";
...
interface ChatRenderCtx {
	block: HTMLElement;
	/** The status row (holds the status text + an optional "Answer now" button). */
	statusEl: HTMLElement;
	/** Text node inside statusEl — updated in place so the sibling "Answer now" button survives. */
	...
	/** tool-call id → its trace box, so out-of-order (concurrent) results land in the right box. */
...
export class LanternView extends ItemView {
	...
	/** "Answer now": graceful wrap-up of the running chat (distinct from the hard Stop). */
	private chatWrapUp: AbortController | null = null;
	...
	private async sendChat(questionArg?: string): Promise<void> {
		...
		this.chatWrapUp = new AbortController();
		this.updateSendButton();
		...
		const statusEl = block.createDiv({ cls: "lantern-chat-status" });
		const statusTextEl = statusEl.createSpan({ text: "Thinking…" });
		this.addAnswerNowButton(statusEl);
	...
	/**
	 * The persistent "Answer now" button: a graceful wrap-up for the running
	 * chat (distinct from the hard Stop). Lives in the status row for the whole
	 * turn; pressing it asks the agent loop to end any in-flight reasoning and
	 * stop issuing tool calls, forcing a final answer at the next opportunity.
	 */
	private addAnswerNowButton(host: HTMLElement): void {
		const btn = host.createEl("button", {
			cls: "lantern-answer-now lantern-chip",
			attr: { "aria-label": "Answer now — stop thinking and searching, answer from what's gathered" },
		});
		createAnswerNowIcon(btn);
		const label = btn.createSpan({ cls: "lantern-answer-now-label", text: "Answer now" });
		btn.setAttr("title", "Stop thinking & searching — answer from what's gathered");
		btn.addEventListener("click", () => {
			this.chatWrapUp?.abort();
			btn.setAttr("disabled", "true");
	...
	private onAgentEvent(event: AgentEvent, ctx: ChatRenderCtx): void {
		switch (event.type) {
			case "status":
				ctx.statusTextEl.setText(event.text);
				break;
			...
			case "answer_delta": {
				if (!ctx.streamEl) {
					...
					ctx.statusTextEl.setText("Writing…");
				}
			...
			case "tool_call": {
				...
				ctx.toolEls.set(event.id, details);
				break;
			}

			case "tool_result": {
				// Map back to this call's box by id — concurrent results can arrive
				// in any order (fall back to the last box for pre-id safety).
				const details = ctx.toolEls.get(event.id) ?? (ctx.traceEl.lastElementChild as HTMLElement | null);
				if (!details) break;
	...
	private renderUsage(ctx: ChatRenderCtx, u: Extract<AgentEvent, { type: "usage" }>): void {
		...
		if (u.tokensPerSecond) parts.push(`${Math.round(u.tokensPerSecond)} tok/s`);
		if (u.cachedTokens) parts.push(`${formatTokens(u.cachedTokens)} cached`);
	...
	private fillAnswerActions(
		...
		if (opts.traceEl) {
			...
			iconBtn(clearTraceIcon(), "Clear reasoning & tools", () => traceEl.empty());
		}
...
/**
 * Icon for the "clear reasoning & tools" action. Prefers Lucide's `brush-cleaning`
 * (added in Lucide 0.505, so only on recent Obsidian) and falls back to `brush`
 * on builds that don't bundle it — otherwise the button would render blank.
 */
let clearTraceIconId: string | null = null;
function clearTraceIcon(): string {
	if (clearTraceIconId === null) {
		const ids = getIconIds();
		clearTraceIconId =
			ids.includes("lucide-brush-cleaning") || ids.includes("brush-cleaning") ? "brush-cleaning" : "brush";
	}
	return clearTraceIconId;
}
